...
//	input [((RESOLUTION == "320x240") ? (7) : (6)):0] y;
	...
	/*
	* Le cambie las coordenadas de x y y para que pinte por toda la pantalla
	*/
	input [9:0] x; 
	input [8:0] y;
	
	/*Direccion de memoria a escribir */
	input [16:0] write_coordinate;
...
//	wire [((RESOLUTION == "320x240") ? (16) : (14)):0] user_to_video_memory_addr; La comente mientras
	 ...
	 */
	 ...
	 wire [16:0] user_to_video_memory_addr;
	/* This bus specifies the address in memory the user must write
	 * data to in order for the pixel intended to appear at location (X,Y) to be displayed
	 * at the correct location on the screen. Aqui le puse fijo la direccion a 16 bits
	 */
	
	//wire [((RESOLUTION == "320x240") ? (16) : (14)):0] controller_to_video_memory_addr; La comente mientras
	/* This bus specifies the address in memory the vga controller must read data from
	 * in order to determine the colour of a pixel located at coordinate (X,Y) of the screen.
	 */
	 
	 wire [16:0] controller_to_video_memory_addr;
	/* This bus specifies the address in memory the vga controller must read data from
	 * in order to determine the colour of a pixel located at coordinate (X,Y) of the screen. Se la puse fija a 16 bits
	 */
	...
	wire [9:0] VGA_R_CONTROLLER;
	wire [9:0] VGA_G_CONTROLLER;
	wire [9:0] VGA_B_CONTROLLER;
...
//	assign writeEn = (plot) & (valid_160x120 | valid_320x240); //Lo comente
	...
	/*****************************************************************************/
	/* Se instancia el generador de texto                                        */
	/*****************************************************************************/
   text_generator generador_texto_unit
      (.clk(CLOCK_50),
       .pixel_x(x_coordinate),
       .pixel_y(y_coordinate), .text_rgb(text_rgb), .text_on(text_on));
		 
		defparam controller.BITS_PER_COLOUR_CHANNEL  = BITS_PER_COLOUR_CHANNEL ;
		defparam controller.MONOCHROME = MONOCHROME;
		defparam controller.RESOLUTION = RESOLUTION;
		
	assign VGA_R = (text_on) ? text_rgb : VGA_R_CONTROLLER;
	assign VGA_G = (text_on) ? text_rgb : VGA_G_CONTROLLER;
	assign VGA_B = (text_on) ? text_rgb : VGA_B_CONTROLLER;
